...
# ══════════════════════════════════════════════════════════════════════════════
# UNIFIED ENGINE (Extracted from core.py for self-containment)
# ══════════════════════════════════════════════════════════════════════════════
    ...
    for it in range(max_iter):
        ...
        if cs <= 10:
            ...
            for v in order:
                ...
                if fixed: break
            if not fixed:
                for _ in range(max(1, int(n*0.02))): sigma[rng.randrange(n)] = rng.randrange(nP)
        ...
        if stall > 100_000:
            ...
            for _ in range(max(1, int(n * (0.05 if cs > 20 else 0.02)))): sigma[rng.randrange(n)] = rng.randrange(nP)
...
def run_fiber_structured_sa(m: int, k: int, seed: int=0, max_iter: int=10_000_000, verbose: bool=True):
    ...
    node_to_key = [((fibers[v],) + tuple(get_coords(v)[1:-1])) for v in range(n)]
